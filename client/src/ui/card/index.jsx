...
function ProductCard({
  ...
  useEffect(() => {
    const checkIfMobile = () => {
      setIsMobile(window.innerWidth <= 688); // Определение ширины экрана для мобильных устройств
    };

    checkIfMobile(); // Проверка при первом рендере
    window.addEventListener('resize', checkIfMobile); // Добавляем слушатель событий для изменения размера экрана
  ...
  return (
    ...
    >
      ...
      {(isHovered || isMobile) && ( // Если пользователь наводит курсор или устройство мобильное
        <div
          className={styles.toggleButtonContainer}
          onClick={handleButtonClick}
        >
